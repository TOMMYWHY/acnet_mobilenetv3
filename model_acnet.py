...
class MobileBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernal_size, stride, nonLinear, SE, exp_size):
        ...
        self.conv = nn.Sequential(
            ...
            G.Grconv(in_channels, exp_size,kernel_size=1, stride=1, padding=0),
        ...
        self.depth_conv = nn.Sequential(
            ...
            #G.Grconv(exp_size, exp_size,kernel_size=kernal_size,stride=stride,padding=padding,groups=exp_size),
            nn.BatchNorm2d(exp_size),
        )
        ...
        self.point_conv = nn.Sequential(
            # nn.Conv2d(exp_size, out_channels, kernel_size=1, stride=1, padding=0),
             G.Grconv(exp_size, out_channels,kernel_size=1, stride=1, padding=0),
         ...
         # org:      3111462
        # ac_3*3:   4974846 todo v1 ------ 
        # ac_1*1:   3792318 todo v2
        # ac_all:   5630782 todo v3 ------
    def forward(self, x):
        # MobileNetV2
        out = self.conv(x)
...
if __name__ =="__main__":
    ...
    model = MobileNetV3_Acnet(model_mode="SMALL", num_classes=1000, multiplier=1.0)
    print(model(temp).shape)
    print(get_model_parameters(model))
    # torch.Size([1, 1000])
    # 18441294 #todo 大6倍。。。
    # 16558886 #todo fc -1 linear
    # 15294974
    #   3111462
# rog:  5152518
    #  14147158
